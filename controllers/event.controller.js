...
module.exports = {
    ...
    put: {
        ...
        registeruser: (req, res, next) => {
            ...
            models.Event.findByIdAndUpdate(id, {$push: {registeredUsers: _id}})
                .then(updatedEvent => {
                    return Promise.all([
                        models.User.findByIdAndUpdate(_id, {$push: {registeredEvents: id}}),
                        models.Event.findOne({_id: updatedEvent._id})
                    ]);
                })
                .then(([userObj, eventObj]) => res.send(eventObj))
        ...
        unregisteruser: (req, res, next) => {
            const id = req.params.id;
            const {_id} = req.user;
            console.log("HEIII OO");

            models.Event.findByIdAndUpdate(id, {$pull: {registeredUsers: _id}})
                .then(updatedEvent => {
                    return Promise.all([
                        models.User.findByIdAndUpdate(_id, {$pull: {registeredEvents: id}}),
                        models.Event.findOne({_id: updatedEvent._id})
                    ]);
                })
                .then(([userObj, eventObj]) => res.send(eventObj))
